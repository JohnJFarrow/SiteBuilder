...
{
    // try and get a handle to a mozilla window and refresh it
    internal class Refresher
    {
        ...
        public static void Refresh()
        {
            ...
            foreach( WindowsSpecific.WindowInformation win in Windows)
            {
                if( win != null && win.Caption.Contains("unrealcode.net") )
                {
                    UnrealCodeWindows.Add(win);
                }
            }

            if(UnrealCodeWindows.Count < 1 )
            {
                Console.WriteLine("Did not find any windows with \"unrealcode.net\" in the title");
            }
            else
            {
                foreach (WindowsSpecific.WindowInformation win in UnrealCodeWindows)
                {
                    SetForegroundWindow(win.Handle);
                    //Refresh the page
                    SendKeys.SendWait("{F5}"); //The page will refresh.
                }
            }
        }
    }
